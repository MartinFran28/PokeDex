...
// Este for recorre los pokemones de la primera generacino y los pasa uno a uno como argumento de la funcion para mostrarlos
for ( let i = 1; i <= 151; i++ ) {
    fetch( URL + i + "/" )
...
botonesAside.forEach( boton => boton.addEventListener( "click", ( event ) => {  // funcion para ejecutar el evento de click en los botones del aside
    ...
    for ( let i = 1; i <= 151; i++ ) {  // Recorriendo todos los elementos nevamente para filtrarlos tipo
        fetch(URL + i)
